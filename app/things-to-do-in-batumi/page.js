...
import { SITE_URL, getRequestLocale, ROUTE_METADATA, buildLocalizedMetadata, getLocalizedHref } from "../lib/siteConfig";
...
export async function generateMetadata() {
  ...
  const meta = ROUTE_METADATA.thingsToDoInBatumi[lang] || ROUTE_METADATA.thingsToDoInBatumi.en;
...
export default async function ThingsToDoInBatumiPage() {
  const [rawPlaces, rawTours, reqHeaders] = await Promise.all([getCachedPlaces(), getCachedTours(), headers()]);
  const lang = getRequestLocale(reqHeaders);
